...
import { clearErrors, getGameDetails, updategame, } from '../../actions/productActions'
...
export const UpdateProduct = () => {
    ...
    const { loading, isUpdated, error: updateError} = useSelector (state => state.game)
    const { error, game} = useSelector ( state => state.gameDetails)
    const productId= params.id;

    useEffect(() => {
        if (game && game._id !==productId){
            dispatch(getGameDetails(productId));
        }else{
            setNombre(game.nombre);
            setPrecio(game.precio);
            setDescripcion(game.descripcion);
            setCategoria(game.categoria);
            setVendedor(game.vendedor);
            setInventario(game.inventario);
            setOldImagen(game.imagen)
        }
        if(error){
            alert.error(error)
    ...
    }, [dispatch, alert, error, isUpdated, updateError, game, productId])
    ...
    const submitHandler = (e) => {
        ...
        dispatch(updategame(game._id, formData))
    }
